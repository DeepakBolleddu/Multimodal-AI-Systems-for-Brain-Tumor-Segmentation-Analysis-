...
OUT_ROOT   = "/g/data/ii16/Image/BrainTumorSeg/Inf_Result/test_use/"
...
from train_model import SSUNet3D
...
DEFAULTS = dict(
    ...
    out_root=OUT_ROOT,
    ckpt=CKPT_PATH,
    in_channels=4, out_channels=1, n_filters=8,
    device_str=None,
...
def load_model(ckpt_path, in_channels, out_channels, n_filters, device):
    model = SSUNet3D()
    state = torch.load(ckpt_path, map_location=device, weights_only=True)  # keep your current loading style
    model.load_state_dict(state)
    model = model.to(device).eval()
    return model

def get_prob_fg_DHW(model, vol_DCHW, device):
    """
    ...
    print("model.training:", model.training)  # expect False

    with torch.no_grad():
        logits = model(vol_NCHWD)                # (1, 1, H, W, D)
...
def main(
    ...
    model = load_model(ckpt, in_channels, out_channels, n_filters, device)
    ...
    for pid in val_ids:
        ...
        prob_fg_DHW = get_prob_fg_DHW(model, vol_DCHW, device)  # (D,H,W)
